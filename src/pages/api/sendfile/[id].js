...
async function readAndWriteFile(singleImg, newPath, query) {
    try {
        ...
        formData.append('type', JSON.stringify({ len: query[0], dig: query[1], lw: query[2], up: query[3], sp: query[4] }))
        const send = await fetch(`http://127.0.0.1:5000/bruteforce/`, {
            method: 'POST',
            body: formData
        })
...
export default async function (req, res) {
    ...
    allFiles.forEach(async f => {
        const pass = readAndWriteFile(f, './public/uploads', id.split('W'))
        passwords.push(pass)
    })
